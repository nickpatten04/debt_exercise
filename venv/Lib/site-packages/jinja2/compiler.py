...
class CodeGenerator(NodeVisitor):
    ...
    def dump_local_context(self, frame):
        return "{%s}" % ", ".join(
            "%r: %s" % (name, target) for name, target in iteritems(frame.symbols.dump_stores())
        )
    ...
    def pop_assign_tracking(self, frame):
        ...
        if public_names:
            if len(public_names) == 1:
                ...
                self.writeline(
                    "context.exported_vars.update((%s))" % ", ".join(imap(repr, public_names))
                )
    ...
    def visit_Template(self, node, frame=None):
        ...
        if have_extends:
            ...
            if supports_yield_from and not self.environment.is_async:
                ...
                self.writeline(
                    ...
                    "root_render_func(context):" % (self.environment.is_async and "async " or "")
                )
                self.indent()
        ...
        for name, block in iteritems(self.blocks):
            self.writeline(
                "%s(context, missing=missing%s):" % (self.func("block_" + name), envenv), block, 1
            )
            self.indent()
            self.write_commons()
        ...
        self.writeline(
            "blocks = {%s}" % ", ".join("%r: block_%s" % (x, x) for x in self.blocks), extra=1
        )
    ...
    def visit_Block(self, node, frame):
        ...
        if supports_yield_from and not self.environment.is_async and frame.buffer is None:
            ...
            self.writeline(
                "%s event in context.blocks[%r][0](%s):" % (loop, node.name, context), node
            )
            self.indent()
    ...
    def visit_Import(self, node, frame):
        ...
        if node.with_context:
            self.write(
                ...
                % (self.environment.is_async and "_async" or "", self.dump_local_context(frame))
            )
        elif self.environment.is_async:
    ...
    def visit_FromImport(self, node, frame):
        ...
        for name in node.names:
            if isinstance(name, tuple):
                ...
                alias = name
            self.writeline(
                "%s = getattr(included_template, " "%r, missing)" % (frame.symbols.ref(alias), name)
            )
            ...
            self.writeline(
                ...
                % (
                    ...
                    "not export the requested name %s" % (self.position(node), repr(name)),
                    name,
                )
            )
            self.outdent()
        ...
        if var_names:
            if len(var_names) == 1:
                ...
                self.writeline(
                    ...
                    % ", ".join("%r: %s" % (name, frame.symbols.ref(name)) for name in var_names)
                )
        if discarded_names:
    ...
    def visit_NSRef(self, node, frame):
        ...
        self.indent()
        self.writeline(
            "raise TemplateRuntimeError(%r)" % "cannot assign attribute on non-namespace object"
        )
        self.outdent()
