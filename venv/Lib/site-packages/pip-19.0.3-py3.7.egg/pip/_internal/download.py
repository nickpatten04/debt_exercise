...
def user_agent():
    ...
    if sys.platform.startswith("linux"):
        ...
        distro_infos = dict(
            filter(lambda x: x[1], zip(["name", "version", "id"], distro.linux_distribution()))
        )
    ...
    if match:
        ...
        if scheme == "file" and comes_from and comes_from.startswith("http"):
            raise InstallationError(
                "Requirements file %s references URL %s, which is local" % (comes_from, url)
            )
        if scheme == "file":
            path = url.split(":", 1)[1]
            path = path.replace("\\", "/")
...
def _copy_file(filename, location, link):
    ...
    if os.path.exists(download_location):
        ...
        if response == "i":
            ...
            logger.warning(
                "Backing up %s to %s", display_path(download_location), display_path(dest_file)
            )
...
def unpack_url(
    ...
    # http urls
        ...
        unpack_http_url(
            link, location, download_dir, session, hashes=hashes, progress_bar=progress_bar
        )
    if only_download:
    ...
    if os.path.exists(download_path):
        ...
        if hashes:
            try:
                ...
                logger.warning(
                    "Previously-downloaded file %s has bad hash. " "Re-downloading.", download_path
                )
