...
from pip._internal.exceptions import InstallationError, InvalidWheelFilename, UnsupportedWheel
from pip._internal.locations import PIP_DELETE_MARKER_FILENAME, distutils_scheme
from pip._internal.models.link import Link
from pip._internal.utils.logging import indent_log
from pip._internal.utils.misc import call_subprocess, captured_stdout, ensure_dir, read_chunks
...
if MYPY_CHECK_RUNNING:
    ...
    from pip._internal.operations.prepare import RequirementPreparer  # noqa: F401
...
def message_about_scripts_not_on_PATH(scripts):
    ...
    not_warn_dirs = [
        os.path.normcase(i).rstrip(os.sep) for i in os.environ.get("PATH", "").split(os.pathsep)
    ]
    ...
    for parent_dir, scripts in warn_for.items():
        ...
        if len(scripts) == 1:
            ...
            start_text = "scripts {} are".format(", ".join(scripts[:-1]) + " and " + scripts[-1])

        msg_lines.append(
            "The {} installed in '{}' which is not on PATH.".format(start_text, parent_dir)
        )
...
def get_csv_rows_for_installed(
    ...
    for row in old_csv_rows:
        if len(row) > 3:
            logger.warning("RECORD line has more than three elements: {}".format(row))
        ...
        for dir, subdirs, files in os.walk(source):
            ...
            for s in subdirs:
                ...
                if is_base and basedir == "" and destsubdir.endswith(".data"):
                    ...
                    and canonicalize_name(s).startswith(canonicalize_name(req.name))
                ):
                    assert not info_dir, (
...
"""
    ...
    if easy_install_script:
        ...
        easy_install_ep = [k for k in console if re.match(r"easy_install(-\d\.\d)?$", k)]
        for k in easy_install_ep:
            del console[k]

    # Generate the console and GUI entry points specified in the wheel
    if len(console) > 0:
        generated_console_scripts = maker.make_multiple(["%s = %s" % kv for kv in console.items()])
    ...
    with open_for_csv(record, "r") as record_in:
        with open_for_csv(temp_record, "w+") as record_out:
            ...
            outrows = get_csv_rows_for_installed(
                reader, installed=installed, changed=changed, generated=generated, lib_dir=lib_dir
            )
...
class Wheel(object):
    ...
    def __init__(self, filename):
        ...
        self.file_tags = {(x, y, z) for x in self.pyversions for y in self.abis for z in self.plats}
...
def should_use_ephemeral_cache(
    ...
    if "binary" not in format_control.get_allowed_formats(canonicalize_name(req.name)):
        logger.info(
            "Skipping bdist_wheel for %s, due to binaries " "being disabled for it.", req.name
        )
        return None
...
class WheelBuilder(object):
    ...
    def _build_one(self, req, output_dir, python_tag=None):
        ...
        with req.build_env:
            return self._build_one_inside_env(req, output_dir, python_tag=python_tag)
    ...
    def _base_setup_args(self, req):
        ...
        # virtualenv.
        return [sys.executable, "-u", "-c", SETUPTOOLS_SHIM % req.setup_py] + list(
            self.global_options
        )
    ...
    def _build_one_pep517(self, req, tempd, python_tag=None):
        ...
        try:
            ...
            wheel_name = req.pep517_backend.build_wheel(
                tempd, metadata_directory=req.metadata_directory
            )
            if python_tag:
                ...
                os.rename(os.path.join(tempd, wheel_name), os.path.join(tempd, new_name))
        ...
        with open_spinner(spin_message) as spinner:
            ...
            try:
                output = call_subprocess(
                    wheel_args, cwd=req.setup_py_dir, show_stdout=False, spinner=spinner
                )
            except Exception:
            ...
            wheel_path = get_legacy_build_wheel_path(
                names=names, temp_dir=tempd, req=req, command_args=wheel_args, command_output=output
            )
            return wheel_path
                ...
                if wheel_file:
                    ...
                    if autobuilding:
                        ...
                        if req.source_dir and not os.path.exists(
                            os.path.join(req.source_dir, PIP_DELETE_MARKER_FILENAME)
                        ):
                            raise AssertionError("bad source dir - missing marker")
                        ...
                        unpack_url(req.link, req.source_dir, None, False, session=session)
                else:
                    build_failure.append(req)
