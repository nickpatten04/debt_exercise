...
class InstallCommand(RequirementCommand):
    ...
    def run(self, options, args):
            ...
            with RequirementTracker() as req_tracker, TempDirectory(
                ...
                try:
                    self.populate_requirement_set(
                        requirement_set, args, options, finder, session, self.name, wheel_cache
                    )
                    ...
                    wb = WheelBuilder(
                        finder, preparer, wheel_cache, build_options=[], global_options=[]
                    )
                    ...
                    if build_legacy:
                        ...
                        # install for those.
                        wb.build(legacy_requirements, session=session, autobuilding=True)
                    ...
                    message = create_env_error_message(error, show_traceback, options.use_user_site)
        ...
        if options.target_dir:
            self._handle_target_dir(options.target_dir, target_temp_dir, options.upgrade)
