...
def compress_for_rename(paths):
    ...
    unchecked = sorted(set(os.path.split(p)[0] for p in case_map.values()), key=len)
    wildcards = set()
...
class UninstallPathSet(object):
    ...
    def remove(self, auto_confirm=False, verbose=False):
        ...
        if not self.paths:
            logger.info(
                "Can't uninstall '%s'. No files were found to uninstall.", self.dist.project_name
            )
            return
    ...
    def from_dist(cls, dist):
        ...
        if not dist_is_local(dist):
            logger.info(
                "Not uninstalling %s at %s, outside environment %s", dist.key, dist_path, sys.prefix
            )
            return cls(dist)
        ...
        if dist_path in {
            ...
            logger.info(
                "Not uninstalling %s at %s, as it is in the standard library.", dist.key, dist_path
            )
            return cls(dist)
        ...
        if (
            ...
            raise UninstallationError(
                ...
                "to it which would lead to only a partial uninstall.".format(dist.project_name)
            )
