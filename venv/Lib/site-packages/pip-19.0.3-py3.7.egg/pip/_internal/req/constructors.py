...
if MYPY_CHECK_RUNNING:
    from typing import Optional, Tuple, Set, Any, Union, Text, Dict  # noqa: F401
...
def parse_editable(editable_req):
    ...
    if url_no_extras.lower().startswith("file:"):
        ...
        if extras:
            return (package_name, url_no_extras, Requirement("placeholder" + extras.lower()).extras)
        else:
    ...
    if is_url(name):
        ...
        if looks_like_dir:
            ...
            if not os.path.isfile(p):
                logger.warning(
                    "Requirement %r looks like a filename, but the " "file does not exist", name
                )
            link = Link(path_to_url(p))
...
def install_req_from_req_string(
    ...
    return InstallRequirement(
        req, comes_from, isolated=isolated, wheel_cache=wheel_cache, use_pep517=use_pep517
    )
