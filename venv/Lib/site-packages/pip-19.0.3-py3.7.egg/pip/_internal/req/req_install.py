...
from pip._internal.locations import PIP_DELETE_MARKER_FILENAME, running_under_virtualenv
...
if MYPY_CHECK_RUNNING:
    from typing import Optional, Iterable, List, Union, Any, Text, Sequence, Dict  # noqa: F401
...
class InstallRequirement(object):
    ...
    def __repr__(self):
        return "<%s object: %s editable=%r>" % (self.__class__.__name__, str(self), self.editable)
    ...
    def load_pyproject_toml(self):
        ...
        pep517_data = load_pyproject_toml(
            self.use_pep517, self.pyproject_toml, self.setup_py, str(self)
        )
    ...
    def run_egg_info(self):
        ...
        if self.name:
            logger.debug(
                "Running setup.py (path:%s) egg_info for package %s", self.setup_py, self.name
            )
        else:
            logger.debug(
                "Running setup.py (path:%s) egg_info for package from %s", self.setup_py, self.link
            )
    ...
    def get_dist(self):
        ...
        if self.metadata_directory:
            ...
            metadata = pkg_resources.PathMetadata(base_dir, self.metadata_directory)
    ...
    def update_editable(self, obtain=True):
        ...
        if not self.link:
            logger.debug(
                "Cannot update repository at %s; repository location is " "unknown", self.source_dir
            )
            return
    ...
    def _clean_zip_name(self, name, prefix):  # only used by archive.
        assert name.startswith(prefix + os.path.sep), "name %r doesn't start with prefix %r" % (
            name,
            prefix,
        )
    ...
    def archive(self, build_dir):
        ...
        if os.path.exists(archive_path):
            ...
            if response == "i":
                ...
                logger.warning(
                    "Backing up %s to %s", display_path(archive_path), display_path(dest_file)
                )
        ...
        with TempDirectory(kind="record") as temp_dir:
            ...
            with open(record_filename) as f:
                for line in f:
                    ...
                    logger.warning(
                        "Could not find .egg-info directory in install record" " for %s", self
                    )
