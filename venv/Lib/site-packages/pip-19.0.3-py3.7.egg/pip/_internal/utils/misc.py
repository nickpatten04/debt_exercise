...
from pip._internal.utils.compat import WINDOWS, console_to_str, expanduser, stdlib_pkgs
...
def get_installed_distributions(
    local_only=True, skip=stdlib_pkgs, include_editables=True, editables_only=False, user_only=False
):
    ...
    return [
        d
        for d in pkg_resources.working_set  # type: ignore
        if local_test(d)
...
def call_subprocess(
    ...
    try:
        proc = subprocess.Popen(
            cmd, stderr=subprocess.STDOUT, stdin=subprocess.PIPE, stdout=stdout, cwd=cwd, env=env
        )
        proc.stdin.close()
    except Exception as exc:
    ...
    if proc.returncode and proc.returncode not in extra_ok_returncodes:
        if on_returncode == "raise":
            ...
            logger.warning(
                'Command "%s" had error code %s in %s', command_desc, proc.returncode, cwd
            )
...
def redact_netloc(netloc):
    ...
    return "{user}{password}@{netloc}".format(
        user=urllib_parse.quote(user), password=password, netloc=netloc
    )
