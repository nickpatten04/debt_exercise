...
def _get_html_page(link, session=None):
    ...
    try:
        ...
        logger.debug(
            "Skipping page %s because it looks like an archive, and cannot " "be checked by HEAD.",
            link,
        )
...
class PackageFinder(object):
    ...
    def __init__(
        ...
        """
        if session is None:
            raise TypeError("PackageFinder() missing 1 required keyword argument: " "'session'")
        ...
        self.secure_origins = [
            ("*", host, "*") for host in (trusted_hosts if trusted_hosts else [])
        ]  # type: List[SecureOrigin]
        ...
        self.valid_tags = get_supported(
            versions=versions, platform=platform, abi=abi, impl=implementation
        )
        ...
        if self.index_urls and self.index_urls != [PyPI.simple_url]:
            lines.append(
                "Looking in indexes: {}".format(
                    ", ".join(redact_password_from_url(url) for url in self.index_urls)
                )
            )
        if self.find_links:
            lines.append("Looking in links: {}".format(", ".join(self.find_links)))
        return "\n".join(lines)
    ...
    def _validate_secure_origin(self, logger, location):
        ...
        for secure_origin in SECURE_ORIGINS + self.secure_origins:
            ...
            try:
                ...
                if (
                    origin[1]
                    and origin[1].lower() != secure_origin[1].lower()
                    and secure_origin[1] != "*"
                ):
                    continue
            else:
    ...
    def find_all_candidates(self, project_name):
        ...
        logger.debug(
            "%d location(s) to search for versions of %s:", len(url_locations), project_name
        )
        ...
        if file_versions:
            ...
            logger.debug(
                "Local files found: %s",
                ", ".join([url_to_path(candidate.location.url) for candidate in file_versions]),
            )
    ...
    def find_requirement(self, req, upgrade):
        ...
        compatible_versions = set(
            req.specifier.filter(
                ...
                prereleases=(self.allow_all_prereleases if self.allow_all_prereleases else None),
            )
        )
        ...
        if installed_version is None and best_candidate is None:
            logger.critical(
                ...
                req,
                ", ".join(sorted({str(c.version) for c in all_candidates}, key=parse_version)),
            )
        ...
        if best_installed:
            ...
            logger.debug(
                ...
                installed_version,
                ", ".join(sorted(compatible_versions, key=parse_version)) or "none",
            )
    ...
    def _link_package_versions(self, link, search):
        ...
        if not version:
            self._log_skipped_link(link, "Missing project version for %s" % search.supplied)
            return None
