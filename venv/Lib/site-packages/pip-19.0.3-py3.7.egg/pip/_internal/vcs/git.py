...
class Git(VersionControl):
    ...
    def get_current_branch(self, location):
        ...
        output = self.run_command(args, extra_ok_returncodes=(1,), show_stdout=False, cwd=location)
        ref = output.strip()
    ...
    def get_revision_sha(self, dest, rev):
        ...
        output = self.run_command(
            ["show-ref", rev], cwd=dest, show_stdout=False, on_returncode="ignore"
        )
        refs = {}
    ...
    def fetch_new(self, dest, url, rev_options):
        ...
        logger.info(
            "Cloning %s%s to %s", redact_password_from_url(url), rev_display, display_path(dest)
        )
    ...
    def get_revision(cls, location, rev=None):
        if rev is None:
            rev = "HEAD"
        current_rev = cls.run_command(["rev-parse", rev], show_stdout=False, cwd=location)
    ...
    def get_src_requirement(cls, location, project_name):
        ...
        req = make_vcs_requirement_url(repo, current_rev, project_name, subdir=subdir)
    ...
    def update_submodules(self, location):
        if not os.path.exists(os.path.join(location, ".gitmodules")):
            return
        self.run_command(["submodule", "update", "--init", "--recursive", "-q"], cwd=location)
    ...
    def controls_location(cls, location):
        ...
        try:
            r = cls.run_command(
                ["rev-parse"], cwd=location, show_stdout=False, on_returncode="ignore"
            )
            return not r
        except BadCommand:
            logger.debug(
