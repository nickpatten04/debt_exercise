...
if MYPY_CHECK_RUNNING:
    from typing import Any, Dict, Iterable, List, Mapping, Optional, Text, Tuple, Type  # noqa: F401
...
class VersionControl(object):
    ...
    def obtain(self, dest):
        ...
        if self.is_repository_directory(dest):
            ...
            logger.warning(
                "Directory %s already exists, and is not a %s %s.", dest, self.name, self.repo_name
            )
        ...
        if response == "s":
            logger.info(
                "Switching %s %s to %s%s", self.repo_name, display_path(dest), url, rev_display
            )
