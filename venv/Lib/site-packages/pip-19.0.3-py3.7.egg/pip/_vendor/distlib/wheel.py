...
class Wheel(object):
    ...
    def metadata(self):
        ...
        with ZipFile(pathname, "r") as zf:
            ...
            if not result:
                raise ValueError(
                    "Invalid wheel, because metadata is " "missing: looked in %s" % ", ".join(fns)
                )
        return result
                ...
                if lib_only:
                    ...
                    if file_version == "1.0":
                        ...
                        try:
                            ...
                            logger.warning(
                                "Unable to read JSON metadata, so " "cannot generate scripts"
                            )
                    if commands:
    ...
    def _get_dylib_cache(self):
        ...
        if cache is None:
            ...
            base = os.path.join(get_cache_base(), str("dylib-cache"), sys.version[:3])
            cache = Cache(base)
        return cache
            ...
            if modified:
                ...
                if dest_dir is None:
                    fd, newpath = tempfile.mkstemp(
                        suffix=".whl", prefix="wheel-update-", dir=workdir
                    )
                    os.close(fd)
                else:
