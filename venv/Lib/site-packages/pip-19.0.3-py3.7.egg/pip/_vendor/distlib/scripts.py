    ...
    def _get_shebang(self, encoding, post_interp=b"", options=None):
        ...
        if self.executable:
            ...
            executable = os.path.join(
                sysconfig.get_path("scripts"), "python%s" % sysconfig.get_config_var("EXE")
            )
        else:  # pragma: no cover
            executable = os.path.join(
                sysconfig.get_config_var("BINDIR"),
                "python%s%s"
                % (sysconfig.get_config_var("VERSION"), sysconfig.get_config_var("EXE")),
            )
        if options:
    ...
    def _copy_script(self, script, filenames):
        ...
        try:
            ...
            if not first_line:  # pragma: no cover
                logger.warning(
                    "%s: %s is an empty file (skipping)", self.get_command_name(), script
                )
                return
