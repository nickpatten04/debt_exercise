...
class LegacyMetadata(object):
            ...
            if name in _PREDICATE_FIELDS and value is not None:
                for v in value:
                    ...
                    if not scheme.is_valid_matcher(v.split(";")[0]):
                        logger.warning(
                            "'%s': '%s' is not valid (field '%s')", project_name, v, name
                        )
            # FIXME this rejects UNKNOWN, is that right?
            elif name in _VERSIONS_FIELDS and value is not None:
                if not scheme.is_valid_constraint_list(value):
                    logger.warning(
                        "'%s': '%s' is not a valid version (field '%s')", project_name, value, name
                    )
    ...
    def check(self, strict=False):
        ...
        for fields, controller in (
            (_PREDICATE_FIELDS, are_valid_constraints),
            (_VERSIONS_FIELDS, scheme.is_valid_constraint_list),
            (_VERSION_FIELDS, scheme.is_valid_version),
        ):
            for field in fields:
