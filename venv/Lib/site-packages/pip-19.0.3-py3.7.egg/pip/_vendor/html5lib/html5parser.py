...
class HTMLParser(object):
    ...
    def mainLoop(self):
        ...
        for token in self.normalizedTokens():
                    ...
                    if (
                        ...
                        or (
                            ...
                            and (
                                (
                                    type == StartTagToken
                                    and token["name"] not in frozenset(["mglyph", "malignmark"])
                                )
            ...
            if (
                type == StartTagToken
                and prev_token["selfClosing"]
                and not prev_token["selfClosingAcknowledged"]
            ):
                self.parseError(
                    "non-void-element-with-trailing-solidus", {"name": prev_token["name"]}
                )
...
def getPhases(debug):
    ...
    class InitialPhase(Phase):
        ...
        def processDoctype(self, token):
            ...
            if (
                name != "html"
                or publicId is not None
                or systemId is not None
                and systemId != "about:legacy-compat"
            ):
    ...
    class BeforeHeadPhase(Phase):
        def __init__(self, parser, tree):
            ...
            self.startTagHandler = _utils.MethodDispatcher(
                [("html", self.startTagHtml), ("head", self.startTagHead)]
            )
    ...
    class InHeadPhase(Phase):
        def __init__(self, parser, tree):
            ...
            self.endTagHandler = _utils.MethodDispatcher(
                [("head", self.endTagHead), (("br", "html", "body"), self.endTagHtmlBodyBr)]
            )
            self.endTagHandler.default = self.endTagOther
    ...
    class InHeadNoscriptPhase(Phase):
        def __init__(self, parser, tree):
            ...
            self.endTagHandler = _utils.MethodDispatcher(
                [("noscript", self.endTagNoscript), ("br", self.endTagBr)]
            )
            self.endTagHandler.default = self.endTagOther
    ...
    class InBodyPhase(Phase):
        ...
        def __init__(self, parser, tree):
            ...
            self.startTagHandler = _utils.MethodDispatcher(
                [
                    ...
                    (("area", "br", "embed", "img", "keygen", "wbr"), self.startTagVoidFormatting),
        ...
        def isMatchingFormattingElement(self, node1, node2):
            return (
                ...
                and node1.namespace == node2.namespace
        ...
        def processCharacters(self, token):
            ...
            if self.parser.framesetOK and any(
                [char not in spaceCharacters for char in token["data"]]
            ):
                self.parser.framesetOK = False
        ...
        def startTagNobr(self, token):
            ...
            if self.tree.elementInScope("nobr"):
                self.parser.parseError(
                    "unexpected-start-tag-implies-end-tag", {"startName": "nobr", "endName": "nobr"}
                )
        ...
        def startTagInput(self, token):
            ...
            if (
                "type" in token["data"]
                and token["data"]["type"].translate(asciiUpper2Lower) == "hidden"
            ):
        ...
        def startTagImage(self, token):
            # No really...
            self.parser.parseError(
                "unexpected-start-tag-treated-as", {"originalName": "image", "newName": "img"}
            )
            self.processStartTag(
                impliedTagToken(
                    "img", "StartTag", attributes=token["data"], selfClosing=token["selfClosing"]
                )
            )
        ...
        def startTagIsIndex(self, token):
            ...
            self.processStartTag(
                impliedTagToken(
                    "input", "StartTag", attributes=attributes, selfClosing=token["selfClosing"]
                )
            )
        ...
        def endTagFormatting(self, token):
            ...
            while outerLoopCounter < 8:
                ...
                if not formattingElement or (
                    formattingElement in self.tree.openElements
                    and not self.tree.elementInScope(formattingElement.name)
                ):
                ...
                while innerLoopCounter < 3:
                    ...
                    self.tree.activeFormattingElements[
                        self.tree.activeFormattingElements.index(node)
                    ] = clone
        ...
        def endTagBr(self, token):
            self.parser.parseError(
                "unexpected-end-tag-treated-as", {"originalName": "br", "newName": "br element"}
            )
            self.tree.reconstructActiveFormattingElements()
    ...
    class TextPhase(Phase):
        ...
        def processEOF(self):
            self.parser.parseError(
                "expected-named-closing-tag-but-got-eof", {"name": self.tree.openElements[-1].name}
            )
            self.tree.openElements.pop()
    ...
    class InTablePhase(Phase):
        ...
        def startTagTable(self, token):
            self.parser.parseError(
                "unexpected-start-tag-implies-end-tag", {"startName": "table", "endName": "table"}
            )
            ...
            if self.tree.elementInScope("table", variant="table"):
                ...
                if self.tree.openElements[-1].name != "table":
                    self.parser.parseError(
                        ...
                        {"gotName": "table", "expectedName": self.tree.openElements[-1].name},
                    )
        ...
        def endTagCaption(self, token):
            if not self.ignoreEndTagCaption():
                ...
                if self.tree.openElements[-1].name != "caption":
                    self.parser.parseError(
                        "expected-one-end-tag-but-got-another",
                        {"gotName": "caption", "expectedName": self.tree.openElements[-1].name},
                    )
    ...
    class InColumnGroupPhase(Phase):
        ...
        def __init__(self, parser, tree):
            ...
            self.startTagHandler = _utils.MethodDispatcher(
                [("html", self.startTagHtml), ("col", self.startTagCol)]
            )
            self.startTagHandler.default = self.startTagOther

            self.endTagHandler = _utils.MethodDispatcher(
                [("colgroup", self.endTagColgroup), ("col", self.endTagCol)]
            )
            self.endTagHandler.default = self.endTagOther
        ...
        def __init__(self, parser, tree):
            ...
            self.endTagHandler = _utils.MethodDispatcher(
                [
                    ...
                    ("table", self.endTagTable),
                    (("tbody", "tfoot", "thead"), self.endTagTableRowGroup),
                    (("body", "caption", "col", "colgroup", "html", "td", "th"), self.endTagIgnore),
                ]
            )
            self.endTagHandler.default = self.endTagOther
    ...
    class InCellPhase(Phase):
        ...
        def startTagTableOther(self, token):
            if self.tree.elementInScope("td", variant="table") or self.tree.elementInScope(
                "th", variant="table"
            ):
                self.closeCell()
                return token
    ...
    class InSelectPhase(Phase):
        ...
        def endTagOptgroup(self, token):
            ...
            if (
                self.tree.openElements[-1].name == "option"
                and self.tree.openElements[-2].name == "optgroup"
            ):
                self.tree.openElements.pop()
    ...
    class InForeignContentPhase(Phase):
        ...
        def processCharacters(self, token):
            if token["data"] == "\u0000":
                token["data"] = "\uFFFD"
            elif self.parser.framesetOK and any(
                char not in spaceCharacters for char in token["data"]
            ):
                self.parser.framesetOK = False
        ...
        def processStartTag(self, token):
            ...
            if token["name"] in self.breakoutElements or (
                token["name"] == "font"
                and set(token["data"].keys()) & set(["color", "face", "size"])
            ):
                self.parser.parseError(
